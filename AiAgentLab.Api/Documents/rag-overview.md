...
context before the model generates its answer.

This is useful when you need answers grounded in private, current, or
domain-specific information that the base model was never trained on — internal
docs, recent research, or notes you keep locally.

A typical RAG pipeline has four stages: chunking (splitting documents into
manageable pieces), embedding (turning each chunk into a vector), storage
(indexing those vectors for fast similarity search), and retrieval (finding the
top-K most relevant chunks for a given query at answer time).
